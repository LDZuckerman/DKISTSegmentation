...
class TestUtils(unittest.TestCase):
    ...
    def test_sav_to_numpy(cls):
        ...
        # positive tests:
        test_type = type(funclib.sav_to_numpy(cls.testfile, cls.test_instrument,
                                      cls.test_band))
    ...
    def test_segment(cls):
        ...
        initial_pixels = sm.all_pixel_indices_from_map(data_map).value # get an array of pixel values
        seg_pixels = sm.all_pixel_indices_from_map(segmented).value
        cls.assertEqual(seg_pixels.shape, initial_pixels.shape)
        ...
        for i in range(len(x)):
            cls.assertNotEqual(data_map.data[x[i], y[i]], segmented.data[x[i], y[i]])

        # error raising tests
        cls.assertRaises(TypeError, funclib.segment, data_map, 'skimage')
        cls.assertRaises(TypeError, funclib.segment, funclib.sav_to_numpy(cls.testfile, cls.test_instrument,
    ...
    def test_get_threshold(cls):
        ...
        # error raising tests   
        cls.assertRaises(ValueError, funclib.get_threshold, test_arr1, 'banana')
